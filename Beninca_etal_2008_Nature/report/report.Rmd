...
```{r}
xout <- seq(min(all.data$Day.number), max(all.data$Day.number), by=3.35)
```

We currently do the fourth root transform before the spline, though Beninca et al do it after. Maybe we can too if we use the same spline function. (Check package pracma for hermitian cubic splines.)

```{r}
all.data$fr.value <- all.data$value^0.25
```
...
```{r}
soi <- "Cyclopoids"
g1 <- ggplot(filter(all.data, variable==soi), aes(x=Day.number, y=fr.value)) +
...
Currently this is done before the spline.

## Detrend

```{r}

final <. group_by(mt, variable) %>%
  do(ksmooth, Day.number, value, kernel="normal", bandwidth=300)

final <- group_by(mt, variable) %>%
  mutate(smoothed=ksmooth(Day.number, value, kernel="normal", bandwidth=300)$y)

soi <- "Cyclopoids"
g1 <- ggplot(filter(final, variable==soi), aes(x=Day.number, y=value)) +
  facet_wrap(~variable, ncol=2, scales="free_y") +
  geom_line(size=0.5, col="red") +
  scale_colour_manual(values = species.colour.mapping)
g2 <- geom_line(data=filter(final, variable==soi), aes(x=Day.number, y=smoothed), size=0.25, col="blue")
g1 + g2


g1 <- ggplot(final, aes(x=Day.number, y=value)) +
  facet_wrap(~variable, ncol=2, scales="free_y") +
  geom_line(size=0.5, col="red") +
  scale_colour_manual(values = species.colour.mapping)
g2 <- geom_line(data=final, aes(x=Day.number, y=smoothed), size=0.25, col="blue")
g1 + g2

final$detrended <- final$value - final$smoothed
...
```{r}
final <- group_by(final, variable) %>%
  mutate(y=scale(detrended))
...
spectra <- final %>% group_by(variable) %>% do(spectra = spectrum(as.ts(.$y), log='no', method="pgram", detrend=F, plot=F))
...
coord_cartesian(ylim=c(0,25), xlim=c(0,240))
...
wspectra <- final %>% group_by(variable) %>% do(spectra = pwelch(as.ts(.$y), window=5, method="pgram", plot=F))
