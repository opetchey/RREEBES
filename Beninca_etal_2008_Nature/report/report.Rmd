...
```{r, echo=FALSE}
tr <- read.csv("~/Dropbox (Dept of Geography)/RREEBES/Beninca_etal_2008_Nature/data/transformed_data_Nature2008.csv",
               skip=7, na.string="", )
...
ggplot(two.datasets, aes(x=Abundance, y=y)) + geom_point()

ggplot(filter(two.datasets, variable=="Rotifers"), aes(x=Day.number, y=Abundance)) + geom_line() +
  facet_wrap(~variable) +
  geom_line(aes(y=y), col="red")

ggplot(two.datasets, aes(x=Day.number, y=Abundance)) + geom_line() +
  facet_wrap(~variable) +
  geom_line(aes(y=y), col="red")
...
## This code reads in data received direct from Stephen Ellner. It is the 
from.steve <- read.csv("~/Dropbox (Dept of Geography)/RREEBES/Beninca_etal_2008_Nature/data/direct from Steve/interp_short_allsystem_newnames.csv")
from.steve <- gather(from.steve, Species, Abundance, 2:13)

g1 <- ggplot(from.steve, aes(x=as.numeric(Day.Number), y=Abundance)) +
  facet_wrap(~Species, ncol=2, scales="free_y") +
  geom_point(size=0.5, col="red") +
  scale_colour_manual(values = species.colour.mapping)
g2 <- geom_line(data=from.steve, aes(x=Day.Number, y=Abundance))
g1 + g2
```
...
This is quite far from the number using code and data from Steve (0.08415112). But recall that the functions have been carefully checked. Probably it is the data going into this function. A final step would be to save the data here, and take it into Steve's function.

# Make the preceding point an issue!!!
...
for(i in 1:length(all.species))
  preds[1,i,] <- predict(all.gams[[i]])[1:679]
...
for(pred.time in 2:12) {
  for(i in 1:length(all.species))
    preds[pred.time,i,pred.time:679] <- predict(all.gams[[i]],
                                                newdata=as.data.frame(t(preds[pred.time-1,,])))[pred.time:679]
}
...
for(i in 1:12) {
  for(j in 1:12) {
    cors[j,i] <- cor(z[1:679,i], preds[j, i, ], use="complete.obs")^2
}}

plot(z[1:679,"Bacteria"],preds[i, "Bacteria", ])
