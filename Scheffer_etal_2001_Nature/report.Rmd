...
```{r}
#ps <- c(seq(1, 5, by=1), 10, 20, 50)
ps <- 10^seq(0, log10(50), length=10)
...
results3 <- expt3 %>%
  group_by(ps) %>%
  do(rez = as.data.frame(get_nl_hyst_by_p(p=.$ps))) %>%
  tidyr::unnest() %>%
  gather(key=Variable, value=Value, 2:4) %>%
...
ggplot(results3, aes(x=ps, y=Value)) +
