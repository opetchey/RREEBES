...
dd_clean <- arrange(dd_clean, Experiment, Block, Time_bin, Species, Replicate)
...
xxx <- cbind(xxx, log(xxx[,c("Algae", "Bacteria", "Ciliate")]))
xxx <- xxx[!apply(sapply(xxx[9:11], is.infinite), 1, function(x) any(x)),]
xxx <- na.omit(xxx)
pairs(xxx[,9:11])
eigen(cor(xxx[,9:11]))
pca1 <- princomp(xxx[,9:11], cor=T)
biplot(pca1)
summary(pca1)
str(pca1)
pca1$loadings
...
```{r}
str(dd_clean)
unique(dd_clean$Time_bin)
qplot(log10(filter(dd_clean, Time_bin==23, Species=="Algae")$density), log10(filter(dd_clean, Time_bin==23, Species=="Bacteria")$density))
...
```{r}
library(mvtnorm)
sigma <- matrix(c(4,2,-2,
                  2,3,-2,
                  -2,-2,5), ncol=3)
sigma
x <- rmvnorm(n=500, mean=c(1,1,1), sigma=sigma)
colMeans(x)
var(x)
pairs(x)

cor_matrix <- cor(x)
eigens <- eigen(cor_matrix)
eigens
