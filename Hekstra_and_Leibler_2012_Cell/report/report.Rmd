...
var_names <- c("Group-Species",
               paste(rep(c("Time", "CPF", "NF"), 10),
                     rep(1:10, each=3), sep="-"))
colnames(revised) <- var_names

#now need to do stack the data
dd <- select(revised, 1:4)
dd$Ecosystem <- 1
names(dd) <- c("Group-Species", "Time", "CPF", "NF", "Ecosystem")
for(i in 2:10) {
  sub_data <- select(revised, 1, (3*(i-1)+2:4))
  sub_data$Ecosystem <- i
  names(sub_data) <- c("Group-Species", "Time", "CPF", "NF", "Ecosystem")
