...
mean_densities <- dd_clean %>% filter(!(Block != "F" & Species == "Bacteria")) %>% group_by(Species, Time_bin) %>% summarize(mean_density = mean(density, na.rm=T))
...
ggplot() + 
  stat_smooth(data=mean_densities, aes(y=mean_density+1, x=Time_bin, group=Species, colour=Species), method="loess", se=F)+ 
scale_colour_manual(values = c("green","red","blue")) +
  geom_ribbon(data=mean_densities,aes(x=Time_bin, ymin=mean_density-boot_sd, ymax=mean_density+boot_sd, group=Species, fill=Species),  alpha=0.2) +
  scale_y_log10() +
