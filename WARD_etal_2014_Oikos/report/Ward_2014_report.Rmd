...
Get an idea of the distribution of time series length (number of observations).

```{r, warning=FALSE, fig.width=10}
ggplot(data=ts %>% group_by(ID_new) %>% summarise(observations=n()), aes(x=observations)) + geom_histogram() 
```
...
#   nnetTs.11 = nnetTs(as.ts(y.train),m=1,d=1,steps=1,size=1)
...
```{r}
ggplot(data=prediction_ASE, aes(x=model_type,y=log(ASE), group=step_ahead, colour=as.factor(step_ahead))) + geom_point() + geom_line() + 
