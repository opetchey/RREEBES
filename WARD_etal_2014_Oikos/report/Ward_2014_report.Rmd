...
set.seed(42)
ggplot(data=subset(ts, ID %in% sample(ts$ID,size=15)), aes(x=Year-mean(Year), y=Value-mean(Value))) + geom_line() + facet_wrap(~Species,ncol=5,nrow=3, scales="free")
```
...
numRec = length(unique(dat$ID)) # number of unique time series 
