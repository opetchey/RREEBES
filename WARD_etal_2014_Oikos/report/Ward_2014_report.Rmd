...
ggplot(data=subset(mean_ASE, step_ahead<5), aes(x=model_type,y=mean_log_ASE, group=step_ahead, colour=as.factor(step_ahead))) + geom_point() + geom_line() + 
  theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=16))

```

Figure 2: Plot MASE for the different species of salmon separately.
```{r}
mean_ASE_salmon <- ddply(subset(models_selected, Class == "Actinopterygii"), .(Database, Species, model_type, step_ahead), summarize, mean_log_ASE = log(mean(ASE, na.rm = T)))

ggplot(data=subset(mean_ASE_salmon, step_ahead<5), aes(x=model_type,y=mean_log_ASE, group=step_ahead, colour=as.factor(step_ahead))) + geom_point() + geom_line() + 
  theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=16)) + facet_wrap(~Species)
```
