---
title: "Reproduce anaylses of Ward et al (2014): Complexity is costly"
...
#install.packages(c("MARSS", "NLTS", "mgcv", "np", "locfit", "forecast", "kernlab", "tsDyn", "ltsa", "timsac", "randomForest"))
library(tsDyn)
library(MARSS)
library(mgcv)
library(np)
...
Explore some time series visually to get an idea of the variability. Year-Mean(Year) and Value - Mean(Value) are plotted to easily compare time series.
...
Run the script to get forecasts based on the training data.
...
Figure 1: Visualize overall forecasting performance using the 13 models specified in the caption. The plot in the manuscript shows only 12 models!
```{r}
#ggplot(subset(models_selected, step_ahead >3), aes(x=log(ASE))) + geom_histogram() + scale_y_log10() + scale_x_log10() + facet_wrap(~model_type)

mean_ASE <- ddply(models_selected, .(Database, model_type, step_ahead), summarize, mean_log_ASE = log(mean(ASE, na.rm = T)))
       
ggplot(data=subset(mean_ASE, step_ahead<5 & mean_log_ASE<60), aes(x=model_type,y=mean_log_ASE, group=step_ahead, linetype=as.factor(step_ahead))) + geom_point() + geom_line() + 
  theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=16)) + scale_linetype_manual(values=c("solid", "dashed", "dotted", "solid")) + facet_wrap(~Database)

```

Figure 2: Plot MASE for the different species of salmon separately. No "Coho" in the current dataset, hence no plot produced. 
```{r}
...
ggplot(data=subset(mean_ASE_salmon_all, step_ahead<5), aes(x=model_type,y=mean_log_ASE, group=as.factor(step_ahead), linetype=as.factor(step_ahead))) + geom_point() + geom_line() + theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=16)) + scale_linetype_manual(values=c("solid", "dashed", "dotted", "solid"))


mean_ASE_salmon <- ddply(subset(models_selected, Species %in% c("Chinook", "Chum", "Pink", "Coho", "Sockeye")), .(Species, model_type, step_ahead), summarize, mean_log_ASE = log(mean(ASE, na.rm = T)))
mean_ASE_salmon$Species <- factor(mean_ASE_salmon$Species ,levels(as.factor(mean_ASE_salmon$Species))[c(3,2,4,5,1)])

ggplot(data=subset(mean_ASE_salmon, step_ahead<5), aes(x=model_type,y=mean_log_ASE, group=as.factor(step_ahead), linetype=as.factor(step_ahead))) + geom_point() + geom_line() + theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=16)) + facet_wrap(~Species) + scale_linetype_manual(values=c("solid", "dashed", "dotted", "solid"))

```

Reproducing Figure 4 based on meta information data set.
```{r}
ggplot(subset(metainfo, Species %in% c("Chinook", "Chum", "Pink", "Coho", "Sockeye")), aes(x=training.acf)) + geom_histogram() + facet_wrap(~Species)
