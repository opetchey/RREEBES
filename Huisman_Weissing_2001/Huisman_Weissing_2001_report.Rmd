...
initial_condition <- expand.grid(sp2=seq(0.00001,2,length.out = 10), sp4=seq(0,2,length.out = 10)) 
...
basins <- lapply(1:nrow(initial_condition), function(x) ode(y = as.numeric(initial_condition[x, ]), times = 1:1000, func = r_star_5))
save(basins, file = "/Users/frank/Documents/Git projects/RREEBES/Huisman_Weissing_2001/basins.RData")

# check one output
matplot(basins[[92]][1:1000 , 2:9], type = "l", xlab = "time", ylab = "Conc",
        main = "5 species R* competition", lwd = 2, col=2:9)
legend("topright", c("sp1", "sp2", "sp3", "sp4","sp5", "R1", "R2", "R3"), col = 2:9, lty = 1)
...
result <- lapply(basins, comp_out, time = 1000, biomass_threshold = 5)

initial_condition$res <- unlist(result)

image(matrix(ifelse(initial_condition$res == "winners: sp1, sp2, sp3", 1,0), byrow = T, ncol = 10))
