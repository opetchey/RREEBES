...
```{r}
five_sp_comp <- read.csv("/Users/Frank/Documents/Git projects/RREEBES/Hsieh_et_al_2008/data/5sp_comp_sim_data_CA_code.csv")
...
Look at state-space plot of the different species:
```{r, state-space, eval=F}
...
embed_sp5 <- Embed(five_sp_comp[,c(i+1)], dimension=3)
...
simplex_mod_stats_sp5_1 <- simplex(five_sp_comp$species.5, lib = c(1, 1000), pred = c(1, NROW(five_sp_comp$species.5)),
...
simplex_mod_pred_sp5_2 <- simplex(five_sp_comp$species.5, lib = c(1, 1000), pred = c(1, NROW(five_sp_comp$species.5)),
                norm_type = c("L2 norm"),  E = 1:10,
...
smap_out <- s_map(five_sp_comp$species.5, lib = c(1, 1000), pred = c(1,NROW(five_sp_comp$species.5)), stats_only = T,
...
block <- block_lnlp(block_data, lib = c(1, 1000), pred = c(1,NROW(block_data)),
  ...
  columns = c("species.3", "species.5",  "species.5m1", "species.5m2", "species.5m3", "species.5m4"), target_column = c("species.3"), stats_only = F, first_column_time = T)
...
plot(sp3_ar_prediction, five_sp_comp[,3],xlab="Prediction using Species 5",ylab="Species 3 (true value)",main="Cross-species AR Model")
abline(a=1, b=1)
